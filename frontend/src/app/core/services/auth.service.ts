...
export class AuthService {
  ...
  login(username: string, password: string): Observable<AuthenticatedUser> {
    return this.http.post<ApiEnvelope<AuthenticatedUser>>(
  ...
  restoreSession(): Observable<AuthenticatedUser | null> {
    ...
    return this.http.get<ApiEnvelope<AuthenticatedUser>>(`${this.baseUrl}/api/v1/auth/me`, { withCredentials: true }).pipe(
      map((response) => response.data),
      tap((user) => {
  ...
  logout(redirect = true): void {
    this.http.post<ApiEnvelope<string>>(`${this.baseUrl}/api/v1/auth/logout`, {}, { withCredentials: true })
